...
import os
import logging
from google import genai  # ИСПРАВЛЕННАЯ СТРОКА (вместо старой)
...
# Новый клиент для работы с Gemini 1.5
...
async def get_ai_prediction(match_data, algo):
    try:
        response = client.models.generate_content(
            ...
            contents=f"Ты эксперт-каппер. Стратегия: {algo}. Матч: {match_data}. Дай совет в 5 словах."
        )
        return f"🤖 {response.text.strip()}"
    except:
        return "🤖 Анализ: Статистика указывает на высокую вероятность гола."
...
async def main():
    while True:
        await check_logic()
        await asyncio.sleep(45) # Оптимальная частота
