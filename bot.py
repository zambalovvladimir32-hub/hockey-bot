...
async def get_data():
    ...
    # Настройка прокси (теперь железно под SOCKS5)
    ...
    async with AsyncSession(impersonate="chrome120") as session:
        try:
            ...
            logger.warning(f"⚠️ Ответ: {resp.status_code}, Размер: {len(resp.text) if resp.text else 0}")
            return None
...
async def main():
    # Проверка переменной при старте
    status_proxy = "ПОДКЛЮЧЕН" if PROXY_URL else "НЕ ВИЖУ ПЕРЕМЕННУЮ PROXY_URL"
    logger.info(f"🚀 СТАРТ. Прокси: {status_proxy}")
    
    while True:
        data = await get_data()
        if data:
            logger.info(f"✅ УСПЕХ! Данные получены ({len(data)} байт)")
            found = parse(data)
            if found:
                await bot.send_message(CHANNEL_ID, "\n\n".join(found[:10]), parse_mode="Markdown")
                logger.info(f"📡 Отправлено в ТГ: {len(found)}")
            else:
                logger.info("🔎 2-й период в лайве пока не найден.")
        else:
            logger.info("⏳ Пробую еще раз через 2 минуты...")
