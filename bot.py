...
logger = logging.getLogger("Hockey1stPeriod_v37.5")
            ...
            while True:
                try:
                    ...
                    for m_block in matches:
                        ...
                        # ПРОВЕРКА: Это точно 1-й период? 
                        # В хоккее после 1-го периода не должно быть счета за 2-й (строка BC÷)
                        if "BC÷" in m_block: 
                            continue # Если есть счет 2-го периода, это уже 2-й перерыв
                        ...
                        if total_goals <= 1:
                            ...
                            if stats and (stats["shots"] >= 11 or stats["pen"] >= 4):
                                ...
                                logger.info(f"💰 ОТПРАВЛЕНО: {h_team}-{a_team}")
