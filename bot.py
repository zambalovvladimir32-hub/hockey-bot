...
logger = logging.getLogger("HockeyV32.4")
...
class HockeySniper:
    def __init__(self):
        ...
        self.headers = {
            ...
            "x-fsign": "SW9D1eZo", # Обновленный ключ
    ...
    async def get_stats(self, session, m_id, h_name):
        # Пробуем несколько разных эндпоинтов для обхода 404/пустых ответов
    ...
    async def run(self):
        logger.info("=== v32.4 DEEP SCAN ЗАПУЩЕН | Пробиваем данные ===")
                        ...
                        for m_block in sec.split('~AA÷')[1:]:
                            ...
                            if self._get_val(m_block, 'AC÷') == '46': # ПЕРЕРЫВ
                                ...
                                if m_id not in self.sent_cache:
                                    ...
                                    if score <= 1: # Наша логика счета
                                        logger.info(f"🎯 Попытка получить статку: {h} - {a}")
                                        stats = await self.get_stats(session, m_id, h)
                                        if stats:
                                            # Наши критерии по броскам (11) и штрафам (4)
                                            ...
                                            logger.warning(f"📭 Сайт скрыл статику для {h}")
