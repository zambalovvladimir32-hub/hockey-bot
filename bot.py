...
logger = logging.getLogger("HockeyFinal_v55.4")
...
class HockeyFinal:
    ...
    async def get_sofa_stats(self, session, h_team, a_team):
        """Улучшенный поиск на SofaScore"""
        try:
            # Чистим от (Rus) и лишних пробелов
            clean_h = re.sub(r'\(.*?\)', '', h_team).strip()
            
            # Пробуем искать только по домашней команде (так выше шанс найти ID матча)
            q = quote(clean_h)
            ...
            for res in data.get('results', []):
                if res.get('type') == 'event':
                    ...
                    if entity.get('sport', {}).get('slug') == 'ice-hockey':
                        # Проверяем, что в этом матче играет и вторая команда (хотя бы по первым 3 буквам)
                        away_in_sofa = entity.get('awayTeam', {}).get('name', '').lower()
                        if a_team[:3].lower() in away_in_sofa or clean_h.lower() in entity.get('homeTeam', {}).get('name', '').lower():
                            event_id = entity['id']
                            break
    ...
    async def run(self):
        logger.info("🦾 ЗАПУСК v55.4: СВЯЗКА FLASH + SOFA (УМНЫЙ ПОИСК)")
        async with AsyncSession() as session:
            while True:
                try:
                    # 1. Flashscore (Радар перерывов)
                    ...
                    if r.status_code == 200 and "¬" in r.text:
                        # Фильтруем именно перерывы (AC÷46)
                        matches = [m for m in r.text.split('~AA÷')[1:] if "AC÷46" in m]
                        logger.info(f"🏟 Найдено {len(matches)} перерывов")
                        ...
                        for m_block in matches:
                            ...
                            if (h_score + a_score) <= 1:
                                ...
                                if stats:
                                    logger.info(f"✅ Стата получена для {h_name}")
