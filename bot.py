...
import os
import logging
from google import genai  # ИСПРАВЛЕННАЯ СТРОКА ПОД ТВОЙ ЛОГ
...
# Твои ключи из Amvera
TOKEN = os.getenv("TELEGRAM_TOKEN")
CHANNEL_ID = os.getenv("CHANNEL_ID")
GEMINI_KEY = os.getenv("GEMINI_API_KEY")

# Новый клиент для работы с Gemini (без красных ошибок)
...
async def get_ai_prediction(match_data, algo):
    try:
        # Новый формат запроса к ИИ
        response = client.models.generate_content(
            model="gemini-1.5-flash",
            contents=f"Ты каппер. Стратегия: {algo}. Матч: {match_data}. Дай короткий совет."
        )
