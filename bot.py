...
async def get_ai_comment(match_info, algo_name):
    try:
        prompt = f"Ты эксперт-каппер. Стратегия: {algo_name}. Матч: {match_info}. Дай совет по ставке в 5 словах."
        response = await asyncio.to_thread(ai_model.generate_content, prompt)
        return f"🤖 {response.text.strip()}"
    except:
        return "🤖 Анализ: Высокая вероятность реализации момента."
                    ...
                    for event in stage.get('Events', []):
                        ...
                        if found_algo:
                            ...
                            if signal_key not in sent_signals:
                                ...
                                comment = await get_ai_comment(f"{t1}-{t2}, счет {s1}:{s2}", found_algo)
...
async def main():
    await bot.send_message(CHANNEL_ID, "🛠 **Обновление: Добой удален.**\nСистема работает по 3 алгоритмам: Засуха, Гол во 2-м, Камбэк.")
    while True:
        await check_logic()
